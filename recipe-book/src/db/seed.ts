...
import { suppressSync, notifyDataChanged } from './sync'
...
export async function seedRecipesIfEmpty(): Promise<void> {
  ...
  await db.open() // гарантируем, что БД открыта (важно на холодном старте)
  ...
  notifyDataChanged() // на случай, если список уже отрисован — обновим его
}
